...
public class PhoneToWatchService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent != null) {
            ...
            final String[] repNames = extras.getStringArray("NAMES");
            Log.d("repNames", repNames.toString());
            final String county = extras.getString("COUNTY");
            Log.d("repCounty", county);
            final String info;
            if (catName.equalsIgnoreCase("Fred") || catName.equalsIgnoreCase("Lexy")) {
                StringBuilder names = new StringBuilder();
                names.append(county);
                names.append('\n' + catName);
                for (String str : repNames) {
                    names.append('\n' + str);
                }
                info = names.toString();
            } else {
