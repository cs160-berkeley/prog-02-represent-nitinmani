...
public class WatchListenerService extends WearableListenerService {
    ...
    private static final String BOB_FEED = "/Bob";
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if( messageEvent.getPath().equalsIgnoreCase( FRED_FEED ) ) {
            ...
            //intent.putExtra("ZIP_CODE",zipcode.getText().toString());
            Log.d("T", "about to start watch MainActivity with CAT_NAME: Lexy");
            startActivity(intent);
        } else if (messageEvent.getPath().equalsIgnoreCase( BOB_FEED )) {
            String value = new String(messageEvent.getData(), StandardCharsets.UTF_8);
            Intent intent = new Intent(this, MainActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            //you need to add this flag since you're starting a new activity from a service
            intent.putExtra("CAT_NAME", "Bob");
            //intent.putExtra("ZIP_CODE",zipcode.getText().toString());
            Log.d("T", "about to start watch MainActivity with CAT_NAME: Bob");
            startActivity(intent);
        } else {
